            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                        this.reply(m.chat, `*「 LIMIT TIDAK CUKUP 」*\nDiperlukan ${plugin.limit} Limit\nLimit Kamu ${_user.limit}\n\nSilahkan Beli Melalui ${usedPrefix}blimit 3`, m)
                        continue // Limit habis
                    }
                    if (plugin.level > _user.level) {
                        this.reply(m.chat, `*「 LEVEL RENDAH 」*\nDiperlukan Level ${plugin.level}\nLevel Kamu ${_user.level}\n\nSilahkan Tingkatkan Level Melalui Menu RPG`, m)
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*「 PERINTAH KHUSUS AUTHOR 」*',
        owner: '*「 PERINTAH KHUSUS OWNER 」*',
        mods: '*「 PERINTAH KHUSUS MODERATOR 」*',
        ...
        restrict: '*「 FITUR INI DI NON-AKTIFKAN 」*'
    }[type]
